...
**** TODO configure Org Ref !

General idea of what is wanted (may be ?):
- Save PDF files at the same place
- Have consistent and somewhat clear bibtex key naming scheme (e.g. no DOI, something like AuthorYEARkeyword)
- Have a file (may be several ?) to store notes about those articles
- Have a way to *search* the list of articles using something else than keys (e.g. authors and title)
  - Have a way to open them
  - or to find them/their path (e.g. to attach them to a PDF)
- Have a way to download PDF from "some places" (may be only Arxiv for now) and format things accordingly
  - Extra: have a way to somewhat automatically do this even when the PDF is downloaded manually
- Useful functions to keep in mind for now: =org-ref-read-key= ...

#+begin_src emacs-lisp :tangle no
  (use-package bibtex-completion
    :custom
    (bibtex-completion-bibliography (expand-file-name "biblio.bib"
                                                      org-directory))
    (bibtex-completion-library-path "~/Documents/Other/articles/")
    (bibtex-completion-notes-template-one-file
     (expand-file-name "biblio-notes.org" org-directory)))
#+end_src
