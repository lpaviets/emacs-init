...
** Eshell crashes Emacs when opened from a PDF ?
Reproduce bug:
- Start an Emacs session
- Open a PDF file
- Use =M-x eshell=. Emacs crashes
- Emacs does not crash if the same buffers are opened, but eshell is invoked from another buffer
** LaTeX
