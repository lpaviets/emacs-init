...
Now, Emacs has a lot of built-in completions styles, telling it how to interpret the input: as a regexp, as initials, as a substring ... We add some other ones, and some possibilities to customize them or even change them on the fly.
...
- Fix company deactivation upon "bad" (?) completion
** LaTeX
...
** PDF
Fix ~pdf-view-restore~ when opening a pdf while already in a pdf-view buffer.
