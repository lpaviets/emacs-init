...
We also add a way to use the mouse for help, with a typical menu appearing when you right-click on something. Of course, when using a keyboard, there are usually more powerful (even contextual) tools, but this is sometimes good enough.
...
Another light-weight option to improve on the built-in ~undo~, which tends to work better than some others choices (notably, =undo-tree=) with some built-in Emacs features such as as "undo-in-region", is =vundo=. It has become much more usable in newer versions of Emacs, where the undo systems has gained some new commands, making it behave more like in most other tools.
...
We add a few functions that are used later in the config. Their purpose is more or less to "prevent" some commands from acting on some regions of the buffer, by deleting them, then processing the file, and inserting them somewhere else afterwards.
...
Although looking at, merging, and dealing with diffs in not strictly related to version control, I believe it is the appropriate time to talk about Ediff. Ediff is (one of the) the built-in modes to visualize side-by-side diffs of different files, and possibly edit and merge them. The defaults are a bit lacking, in particular when it comes to layout/UI, so this is one of the main changes we do.
...
For a more recent and, in some settings, featureful debugging experience, there is the DAP protocol, playing the same role than LSP for the programming experience (auto-completion, jump-to definition/reference, etc).

The general protocol exists independently from Emacs, and we therefore have to choose a client for it, just as we do with LSP. We choose ~dape~, which is the "closest to built-in" compared to the alternative ~dap-mode~ (same difference than in ~eglot~ vs ~lsp-mode~).
...
Graphviz is a simple tool to visualize graphs. It used a custom file format, usually withe extension ~.dot~.
...
A useful feature, offered by some static website generators (e.g. [[https://getpelican.com/][Pelican]] ...), consists in defining some "abbreviations" along with a description, all occurrences of which being replaced in the exported document by a HTML ~abbr~ element. This is a built-in feature of Org-mode, but is easily added, using ~org-export-before-parsing-hook~, regular expressions tricks, and the alread-existing Org parsing capabilities.
...
By default, Org exports automatically generated section names for healines, section and so on. This means in particular that the generated IDs are modified each time the file is re-exported, and that it is also meaningless. This code tries to fix it, by generating IDs depending on the current section, possibly using sections higher up in the tree until the ID is unique. The code is taken, and might be adapted, from [[https://github.com/alphapapa/unpackaged.el?tab=readme-ov-file#export-to-html-with-useful-anchors][Alphapapa's list of unpackaged but still useful functions]].
...
Sometimes, we don't only want to view PDF, but also to modify them, for example split, concatenate, extract pages ... or add a table of contents.

Once again, we rely on external CLI tools and some wrapper Emacs package that make using those tools easier.

*MANUAL* As explained on [[https://github.com/dalanicolai/doc-tools-toc][the package webpage]], you need to install some utilities: ~pdftotext~ from poppler-utils, ~pdfoutline~ from fntsample, and ~tesseract~ (sometimes packages as ~tesseract-ocr~).
...
Another useful thing is to view the entire bibliography as a table, with more or less information: this makes it easier to filter, sort, navigate bibliography possibly split in several files.
...
***** Multibib
There are various packages to deal with having multiple bibliography in a file (not /input/ files, which is handled by LaTeX anyway, and for which RefTeX is fine, but in the /output/). One of those packages is =multibib=, and as it works quite well and as there is already a style file for it, I will use this one and improve on it.

The biggest problem is that =multibib= creates several =.aux= files, which Emacs knows nothing about, and so RefTeX does not pickup on them and does not automatically compile them to the appropriate =.bbl=.
We override this behaviour as follows:
- Create a new function to compile the "master file", running BibTeX "as usual" but changing the /following/ operations by using another sentinel function. This modified BibTeX function will override the usual ~TeX-run-BibTeX~.
- Make other functions to deal specifically with the bibliography =.aux= files generated by the =multibib= packages. Those functions will be called repeatedly, processing one file at a time until they are all compiled.

#+NAME: bibtex-multibib
#+begin_src emacs-lisp :tangle no
  (defun lps/TeX-LaTeX-sentinel (process name)
  ...
  errors or warnings to show."
    ...
    (cond ((TeX-TeX-sentinel-check process name))
          ;; Change things here: compile all extra bib files
          ;; Somewhat hacky: just compile everything anyway, regardless of
          ;; modification time
          ((save-excursion
             (re-search-forward
  ...
  (defun lps/LaTeX-multibib-all-newcites ()
    (let (suffix)
  ...
  ;; Should maybe be an advice ?
  (defun TeX-run-multibib-BibTeX-main (name command file)
    "Create a process for NAME using COMMAND to format FILE with BibTeX."
    (message "Running on main")
    (let ((process (TeX-run-command name command file)))
      (setq TeX-sentinel-function #'TeX-multibib-BibTeX-main-sentinel)
      (if TeX-process-asynchronous
          process
        (TeX-synchronous-sentinel name file process))))

  (defun TeX-multibib-BibTeX-main-sentinel (_process _name)
    (TeX-BibTeX-sentinel _process _name)
    (setq TeX-command-next "BibTeX-multibib"))

  (defun TeX-run-multibib-BibTeX-others (name command file)
    (let ((newcites (lps/LaTeX-multibib-all-newcites)))
      (message "Running BibTeX on files %s" newcites)
      (dolist (other newcites)
        (let ((TeX-master nil)
              (TeX-transient-master other))
          (TeX-run-BibTeX name command other)))))

  (defun lps/LaTeX-multibib-BibTeX-fixes ()
    (setq-local TeX-command-BibTeX "BibTeX-main")
    (setq-local TeX-sentinel-default-function 'lps/TeX-LaTeX-sentinel)
    (setq-local TeX-command-list
                (let ((TeX-new-command-list TeX-command-list))
                  (dolist (new-bibtex '(("BibTeX-multibib" "bibtex " TeX-run-multibib-BibTeX-others nil
                                         (plain-tex-mode latex-mode doctex-mode ams-tex-mode texinfo-mode
                                                         context-mode)
                                         :help "Run BibTeX (multibib-others)")
                                        ("BibTeX-main" "bibtex %(O?aux)" TeX-run-multibib-BibTeX-main nil
                                         (plain-tex-mode latex-mode doctex-mode ams-tex-mode texinfo-mode
                                                         context-mode)
                                         :help "Run BibTeX (multibib-main)")))
                    (unless (member new-bibtex TeX-new-command-list)
                      (push new-bibtex TeX-new-command-list)))
                  TeX-new-command-list)))

  (add-hook 'TeX-update-style-hook
            (lambda ()
              (TeX-add-style-hook "multibib" 'lps/LaTeX-multibib-BibTeX-fixes)))
#+end_src
...
[[https://github.com/alexluigit/dirvish/][Dirvish]] is (almost) a drop-in replacement for Dired. As it is essentially a big wrapper around the built-in Dired, most of the customizations (bindings, some defaults ...) are kept ! However, you may find that some of your old customizations have become unneeded, for Dirvish provides more options, or already has the "good" default behaviour, and so on.

*MANUAL* To enable all the options provided by Dirvish, you need to install the =fd= and =mediainfo= command-line tools.
...
Being used to write any kind of text into Emacs, it is probably easier to do so /everywhere/ ! We use the following package, implemented in Elisp (and so, not requiring to install any external tool besides the dependencies), to be able to use Emacs to fill any text field in any application.
...
The actual mail client that we use is a frontend to =mu=, called [[https://github.com/djcb/mu/tree/master/mu4e][mu4e]]. Depending on your Linux distribution, you might encounter some troubles with the following code, as the syntax might have changed between the versions and I might have modified internal functions that no longer exist (or do not yet exist in your version !).
...
In order to provide contextual mail completion, address, server synchronization ... mu4e relies on /contexts/, essentially mappings between local maildirs, mail addresses, and a bunch of local variables that are indenpendently setup per context.
...
As ~mu4e~ provides a lot of query options to filter mails, sometimes you can forget what the syntax is. To make some non-trivial queries easier to write, we add some interactive functions for the most common options.
...
Sometimes, people send calendar invitations via Email. Most of the time, this means that there is a =text/calendar= attachment to the message, and luckily, Gnus already knows how to deal with them ! It can also be integrated with org-agenda; however, I find this integration a bit lacking, and override some of the built-in functions.
...
Emacs comes with a few ways to do spell checking and does a good job at highlighting mistakes with /e.g./ =flyspell-mode=.
...
We add a few wrappers around this built-in functionality, mostly for convenience.
...
Because we might read and write text in several languages, we use a package which will try to automatically detect which language the current buffer uses, and switch dictionaries accordingly.
...
Moreover, it is sometimes useful to see a full-fledged, full-screen, complete calendar. For this, we use Calfw, a "calendar framework for Emacs". It is less useful than ~org-agenda~ for integrating with Org-mode (priorities, TODOs, tags and so on), but is a very nice frontend/visualisation tool.

In particular, it integrates somewhat nicely with ~org-agenda~: it can read the content of the agenda, using internally ~org-agenda-get-day-entries~, and so any modification done to ~org-agenda~ "sources" will appear in Calfw. For example, enabling the diary in Org /will/ show the diary entries (holidays, etc ...) in Calfw.

Although not been updated in the last 7 years at the time of writing (2024), it is still quite featureful and stable, and does not have that many bugs - moreover, it is quite well documented, and any break can probably be fixed by an Emacs user.
...
** Org mode
- [X] Fix (or check what breaks if fix) ~org-element-latex-fragment-parser~ so that it recognizes "dollars" followed by a dash directly (and no space+dash).
...
- [X] Fix HTML files opening in Chrome sometimes (somewhat complicated. Some keywords: mailcap, sensible-browser, the /etc/mailcap file).
...
- Maybe already done: add a way to jump /from/ the agenda /to/ a Cafw buffer (the other direction is already implemented, I believe) displaying "the same content" (if both directions are unimplemented: do them both ?)
- Improve colours of the displayed items in Calfw ? Maybe reuse ~org-agenda-category-icon-alist~, or other display code from the agenda ? Likely function to modify/variable to set: ~cfw:org-schedule-summary-transformer~.
- Add a way to export ~.ics~ invitations from =mu4e= to the agenda
  - Partially done in the file =gnus-icalendar=, e.g. see the function ~gnus-icalendar-sync-event-to-org~ and the variable ~gnus-icalendar-org-enabled-p~
- Check that the "useful IDs" generated in the HTML export also work in LaTeX exports (or any other, really), and fix it otherwise. Probably feasible using a toggle of the mode.
** Printing
...
** PDF/LaTeX
*** Styles
- Add a style for =multibbl=, probably using the one built-in for =multibib=.
- For =multibbl=:
  - Fix compilation, some bibliography files need to be compiled "manually" files (not that easy, nowhere to cleanly hook to). The simplest (but unclear if it is enough/reliable enough ?) would be to change ~TeX-command-BibTeX~ when using the package, and to add a corresponding entry/function in ~TeX-command-list~ doing the appropriate thing.
  - Fix completion with reftex-cite: only use "correct" biblio file.
*** Documentation
...
- Fix dirvish (not only ? Same bug in =emacs -Q= and dired ...) and (some ?) FTP hanging forever.
  - Seems like it came from a symlink to a file not accessible by the FTP
