...
- Fix ~compile-multi~ to default to ~comint~ when called with a "run" command,
  typically defined by ~projection-multi-compile~: as of <2024-11-07 Thu>, the
...
#+end_src
The ~compile-cmd~ variable has the necessary information (in its string
properties) to know what "type" of command it is, but there is no easy advice
here.
