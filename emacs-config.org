...
Emacs can be run in a terminal, with no GUI window. This is sometimes useful,
but there are some rough edges. Most customizations work fine both in a GUI and
in a TTY, but special care needs to be taken for some stuff, especially when
dealing with fancy fonts and complex inputs. The standard way to wrap code meant
for GUI is to use the ~(display-graphic-p)~ function, but there are a few special
hooks run when we are in a terminal that we can use to do stuff precisely /when/
we are in a TTY.
...
We also add some configuration to navigate more easily between several windows,
without having to spam the default binding for ~other-window~ (~C-x o~).
...
Finally, we set up general parameters related to all windows in Emacs: their
height, how we can split them, etc
...
Even without XWidget support, Emacs can still do basic HTML rendering, using
~shr~, which can be useful when viewing Emails for example.
...
In fairness, Emacs already comes with a variety of ways to mark "stuff" of
varying nature, using the "thing at point" mechanism. We extend it to be able to
quickly mark "something" (being prompted for the thing) without remembering each
~mark-<X>~ function.
...
We first configure some time-based utilities to be able to parse French days,
etc.
...
using an european style, and to use "French holidays" -- the above code is only
used to /read/ dates in some functions.
