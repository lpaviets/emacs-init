...
LSP to use. We use the one called [[https://github.com/MaskRay/ccls/wiki/lsp-mode][ccls]].

*MANUAL* To use the ccls server, follow the instruction [[https://github.com/MaskRay/ccls/][here]], or simply install
the available version from your distribution standard repositories.
...
#+begin_src emacs-lisp :tangle no
  ...
  (defun pdf-view-timetrack-page-change ()
    (let ((current-pagelabel (pdf-view-current-pagelabel)))
      (when (/= pdf-view-timetrack-last-pagelabel
                current-pagelabel)
        (pdf-view-timetrack-log pdf-view-timetrack-last-pagelabel
                                (- (current-time) pdf-view-timetrack-last-time))
