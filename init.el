...
If TEXT does not have a range, return nil."
  ...
  ;; with Emoji symbols.
  ;; More precisely, the internal function `cfw:render-truncate' calls `truncate-string-to-width',
  ...
  (defun cfw:truncate-string (string width ellipsis)
    (let ((i 1)
          (olds "")
  ...
  (defun-override lps/cfw:render-truncate (org limit-width &optional ellipsis)
...
internals of character width/display width/etc."
    ...
    (if (< limit-width (string-width org))
        (let ((str (cfw:truncate-string org limit-width ellipsis)))
